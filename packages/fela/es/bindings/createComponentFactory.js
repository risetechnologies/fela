...
export default function createComponentFactory(createElement, contextTypes) {
  var withProxy = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : false;
  ...
  return function createComponent(rule) {
    ...
    var usedProps = withProxy ? extractUsedProps(rule) : {};
    var FelaComponent = function FelaComponent(_ref, _ref2) {
      ...
      var resolvedPassThrough = [].concat(_toConsumableArray(resolvePassThrough(passThroughProps, ruleProps)), _toConsumableArray(resolvePassThrough(passThrough, ruleProps)), _toConsumableArray(withProxy ? resolveUsedProps(usedProps, ruleProps) : []));
